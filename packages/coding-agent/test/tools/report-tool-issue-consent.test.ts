...
function toolSession(settings: Settings, cwd: string): ToolSession {
	return {
		cwd,
		hasUI: false,
		getSessionFile: () => null,
...
describe("resolveAutoQaConsent", () => {
	...
	it("does not create a local grievance when consent is dismissed", async () => {
		const originalAgentDir = getAgentDir();
		const tempDir = TempDir.createSync("@pi-autoqa-consent-");
		try {
			...
			const tool = createReportToolIssueTool(toolSession(settings, tempDir.path()), ["read"]);
			await tool.execute("call-1", { tool: "read", report: "selector ignored" });
			await Promise.resolve();
			await Promise.resolve();

			expect(calls).toBe(1);
			expect(settings.get("dev.autoqaConsent")).toBe("unset");
			expect(await Bun.file(getAutoQaDbPath()).exists()).toBe(false);
		} finally {
